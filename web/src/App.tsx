...
export default function App() {
  ...
  const defaultBase = servedByEngine ? `${window.location.origin}/v1` : "http://127.0.0.1:8080/v1"
  const [baseUrl, setBaseUrl] = useState(() => {
    const saved = stored(localStorage, "tinllm.baseUrl", defaultBase)
    if (servedByEngine && saved === "http://127.0.0.1:8080/v1" && defaultBase !== saved) return defaultBase
    return saved
  })
  const [apiKey, setApiKey] = useState("")
  const [models, setModels] = useState<string[]>([])
  const [model, setModel] = useState(() => stored(localStorage, "tinllm.model", "tinllm-moe"))
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-row">
          ...
          <div><h1>TinLLM</h1><p>Tiny engine, immense model</p></div>
        </div>
      ...
      <main className="chat-panel">
        ...
        <div className="conversation">
          {!messages.length ? (
            <div className="empty-state">
              ...
              <span className="eyebrow">TINLLM ENGINE</span>
              <h2>Ask the giant.<br /><em>Keep the machine yours.</em></h2>
              <p>Connect to a local TinLLM server and stream responses directly from your hardware. Nothing leaves the endpoint you choose.</p>
            ...
            <div className="message-list">
              {messages.map((item) => (
                <article key={item.id} className={cn("message", item.role)}>
                  ...
                  <div><div className="message-meta">{item.role === "user" ? "You" : "TinLLM"}</div><div className="message-body">{item.content || <span className="typing" aria-label="Generating"><i /><i /><i /></span>}</div></div>
                </article>
              ))}
              <div ref={bottomRef} />
            </div>
        ...
        <div className="composer-wrap">
          ...
          <div className="composer">
            <Textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Message TinLLM…" onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send() } }} />
