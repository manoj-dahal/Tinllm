...
import { BrainCircuit, Flame, Layers, Zap } from "lucide-react"
...
const TIER_NAME = ["NVMe Disk Direct", "RAM Pinned", "VRAM Resident"]
...
function depthRole(row: number, rows: number, isMtp: boolean): string {
  if (isMtp) return "MTP Head — Multi-Token Prediction Speculative Decoder"
  const f = row / Math.max(rows - 1, 1)
  if (f < 0.2) return "Early Layers — Surface Tokens, Spelling & Syntax Features"
  if (f < 0.45) return "Lower-Middle Layers — Phrase Structure & Direct Knowledge Retrieval"
  if (f < 0.7) return "Upper-Middle Layers — Complex Reasoning & Cross-Attention Semantics"
  if (f < 0.9) return "Late Layers — Long-Range Planning & Coherence Structuring"
  return "Final Output Layers — Distribution Shaping & Next Token Logit Selection"
}
  ...
  return (
    <div className="brain-page">
      <div className="brain-head">
        <div className="section-title"><BrainCircuit className="size-4" /> TinLLM Active Expert Visual Cortex — {data ? `${data.rows} Layers × ${data.cols} MoE Experts` : "Connecting to C Engine..."}</div>
        <div className="brain-legend">
          ...
          <span><i style={{ background: "#3a4750" }} /> NVMe Disk {totals[0].toLocaleString()}</span>
          <span><Flame className="size-3" /> Brightness = Routing Heat</span>
          <span className="brain-pulse-hint"><Zap className="size-3" /> White Flash = Top-8 Active Route</span>
        </div>
      </div>
      <div className="brain-canvas-wrap" ref={wrapRef}>
        <canvas ref={canvasRef} onMouseMove={onMove} onMouseLeave={() => setTip(null)} />
        {!connected && <p className="runtime-unavailable">Connect to TinLLM REST Server to render live neural cortex routing.</p>}
      </div>
      {tip && data && (() => {
        ...
        <div className="brain-tip" style={{ left: tip.x + 14, top: tip.y + 14 }}>
          <div className="brain-tip-title"><Layers className="size-3" /> Layer {realLayer}{isMtp ? " (MTP Speculation)" : ""} · Expert #{tip.col}</div>
          <div>Memory Tier: <strong style={{ color: ["#8b9aa3", "#5a9bd8", "#4ed6a5"][tip.tier] }}>{TIER_NAME[tip.tier]}</strong></div>
          <div>Routing Frequency: <strong>{tip.heat === 0 ? "Inactive / Cold" : `~2^${tip.heat} activations`}</strong></div>
          {entry ? <>
            <div className={entry.label.startsWith("specialist") ? "brain-tip-spec" : undefined}>
              {entry.label.startsWith("specialist") ? `⭐ Specialist: ${entry.top}` : "Generalist Domain"}
