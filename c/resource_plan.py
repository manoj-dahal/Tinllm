#!/usr/bin/env python3
"""Hardware and model placement planning for tinllm's disk/RAM/VRAM tiers."""
...
def environment_for_plan(plan, env=None, cuda_enabled=True):
    ...
    if not cuda_enabled or not devices or vram["budget_bytes"] <= 0:
        return result
    if result.get("TIN_CUDA", "1") == "0":
        return result

    result.setdefault("TIN_CUDA", "1")
    if "TIN_GPU" not in result and "TIN_GPUS" not in result:
        key = "TIN_GPU" if len(devices) == 1 else "TIN_GPUS"
