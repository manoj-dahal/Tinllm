...
def main():
    ...
    if a.selftest_nvfp4:
        ...
        assert maxerr < 1e-9
        # 3) requant tinllm int4 su valori dequantati -> errore piccolo atteso
        ...
        print(f"[nvfp4] dequant->tinllm int4->dequant: errore rel medio = {rel:.4f} "
    ...
    def download_retry(repo, fn, dest, tries=999):
        ...
        def worker(t):
            ...
            while done[t] < s1 - s0 and not stopfail:
                pos = s0 + done[t]
                req = urllib.request.Request(url, headers={"User-Agent": "tinllm-convert",
