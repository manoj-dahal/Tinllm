# warmup.ps1 - overnight expert-cache warmup for tinllm
#
...
"tinllm warmup - started $start"        | Tee-Object -FilePath $Log -Append
...
for ($r = 1; $r -le $Rounds; $r++) {
    for ($i = 0; $i -lt $Prompts.Count; $i++) {
        ...
        $t0 = Get-Date
        # tin run writes status to stderr (normal) and may exit non-zero on
...
"$line"                                           | Tee-Object -FilePath $Log -Append
"tinllm warmup - finished $end"                  | Tee-Object -FilePath $Log -Append
"  duration:    $duration"                        | Tee-Object -FilePath $Log -Append
"  selections:  $baseline -> $final (+$gain)"     | Tee-Object -FilePath $Log -Append
"  next: python tin chat --model $Model"         | Tee-Object -FilePath $Log -Append
"$line"                                           | Tee-Object -FilePath $Log -Append
