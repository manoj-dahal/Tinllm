...
static DeviceContext g_ctx[TIN_CUDA_MAX_DEVICES];
static int g_nctx;
...
extern "C" int coli_cuda_init(const int *devices, int count) {
    int available = 0;
    if (!devices || count < 1 || count > TIN_CUDA_MAX_DEVICES) return 0;
...
extern "C" int coli_cuda_expert_group(ColiCudaTensor *const *gates,
    ...
    int async=!getenv("TIN_CUDA_ASYNC")||atoi(getenv("TIN_CUDA_ASYNC"));
    ...
    int profile=getenv("TIN_CUDA_PROFILE")&&atoi(getenv("TIN_CUDA_PROFILE"));
    cudaEvent_t ev[4]={};
    ...
    int tc=getenv("TIN_CUDA_TC_INT4")&&atoi(getenv("TIN_CUDA_TC_INT4"));
    tc=tc&&all_s4&&D%32==0&&I%32==0&&D%8==0&&I%8==0;
    int tc_min=getenv("TIN_CUDA_TC_MIN_ROWS")?atoi(getenv("TIN_CUDA_TC_MIN_ROWS")):8;
    ...
    if(tc){
    ...
    }else if(all_s4&&ctx->compute_major>=7&&getenv("TIN_CUDA_TC_W4A16")&&
             atoi(getenv("TIN_CUDA_TC_W4A16"))){
        ...
        int tc16_min=getenv("TIN_CUDA_TC_W4A16_MIN")?atoi(getenv("TIN_CUDA_TC_W4A16_MIN")):16;
        int off16=0;
        for(int c=0;c<count;c++){
            int r=rows[c];
    ...
    }else if(all_s4&&(!getenv("TIN_CUDA_W4_PACKED")||atoi(getenv("TIN_CUDA_W4_PACKED")))){
        dim3 hg((unsigned)I,(unsigned)max_rows,(unsigned)count),og((unsigned)D,(unsigned)max_rows,(unsigned)count);
        int dual=!getenv("TIN_CUDA_DUAL_PROJ")||atoi(getenv("TIN_CUDA_DUAL_PROJ"));
