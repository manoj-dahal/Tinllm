#!/usr/bin/env python3
"""Dependency-free OpenAI-compatible HTTP gateway for the tinllm engine."""
...
def content_text(content, param):
    ...
    for index, part in enumerate(content):
        if not isinstance(part, dict) or part.get("type") not in ("text", "input_text"):
            raise APIError(400, "TinLLM currently supports text message content only.",
...
def generation_options(body, limit):
    if body.get("n", 1) != 1:
        raise APIError(400, "TinLLM currently supports `n=1` only.", "n", "unsupported_value")
...
def read_engine_turn(stream, sentinel, on_bytes):
    ...
    while True:
        byte = stream.read(1)
        if byte == b"":
            raise RuntimeError("tinllm engine exited unexpectedly")
        pending += byte
...
class Engine:
    def __init__(self, executable, model, cap=8, max_tokens=1024, env=None, kv_slots=1):
        ...
        self.dispatcher = threading.Thread(target=self._dispatch_stdout,
                                           name="tinllm-stdout", daemon=True)
    ...
    def generate(self, prompt, max_tokens, temperature, top_p, on_text, cache_slot=0,
        ...
        with self.pending_lock:
            if self.closed:
                raise RuntimeError("tinllm engine is shutting down")
            if self.dispatcher_error is not None:
                raise RuntimeError("tinllm engine dispatcher stopped") from self.dispatcher_error
            if self.process.poll() is not None:
                raise RuntimeError("tinllm engine is not running")
    ...
    def close(self):
        with self.pending_lock:
            if self.closed:
                return
            self.closed = True
        self._fail_pending(RuntimeError("tinllm engine is shutting down"))
...
def model_object(model_id, created):
    return {"id": model_id, "object": "model", "created": created, "owned_by": "tinllm"}
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def send_cors_headers(self):
        ...
        self.send_header("Access-Control-Expose-Headers",
                         "x-request-id, x-tinllm-queue-wait-ms, Retry-After")
    ...
    def do_POST(self):
        ...
        try:
            ...
            api_error = APIError(500, "The tinllm engine failed to process the request.",
    ...
    def generation(self, body, prompt, request_id, chat):
        ...
        with self.server.scheduler.admit(self.client_disconnected, cache_slot) as admission:
            ...
            queue_headers = {"x-tinllm-queue-wait-ms": str(round(queue_wait * 1000))}
            if not stream:
                output = []
    ...
    def completion(self, body, request_id):
        ...
        if not isinstance(prompt, str):
            raise APIError(400, "TinLLM currently requires `prompt` to be a string.", "prompt")
        if not prompt:
            raise APIError(400, "`prompt` must not be empty.", "prompt")
        self.generation(body, prompt, request_id, False)


def serve(model, host="127.0.0.1", port=8000, model_id="glm-5.2-tinllm", api_key=None,
    ...
    if host not in ("127.0.0.1", "localhost", "::1") and not api_key:
        print("WARNING: API is listening beyond localhost without TIN_API_KEY", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--model", default=os.environ.get("TIN_MODEL"), required=not os.environ.get("TIN_MODEL"))
    parser.add_argument("--engine", default=str(HERE / "glm"))
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8000)
    parser.add_argument("--model-id", default=os.environ.get("TIN_MODEL_ID", "glm-5.2-tinllm"))
    parser.add_argument("--api-key", default=os.environ.get("TIN_API_KEY"))
    ...
    parser.add_argument("--max-queue", type=int, default=int(os.environ.get("TIN_MAX_QUEUE", "8")))
    parser.add_argument("--queue-timeout", type=float,
                        default=float(os.environ.get("TIN_QUEUE_TIMEOUT", "300")))
    parser.add_argument("--kv-slots", type=int, default=int(os.environ.get("TIN_KV_SLOTS", "1")))
