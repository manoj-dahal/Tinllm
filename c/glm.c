...
static int g_metal_gemm_min=16;   /* TIN_METAL_GEMM_MIN: min rows to send a matmul_qt GEMM to GPU */
...
#ifdef TIN_CUDA
    ColiCudaTensor *kv_b_shard[TIN_CUDA_MAX_DEVICES];
    int shard_h0[TIN_CUDA_MAX_DEVICES],shard_hn[TIN_CUDA_MAX_DEVICES],n_kv_b_shard;
...
static int g_cuda_devices[TIN_CUDA_MAX_DEVICES], g_cuda_ndev, g_cuda_rr;
static int64_t g_cuda_dense_projected[TIN_CUDA_MAX_DEVICES];
...
static void cuda_stats_print(void){
    ...
    if(calls) fprintf(stderr,"[CUDA] expert groups: %llu call, %llu expert, %llu righe "
        ...
        getenv("TIN_CUDA_PROFILE")?"; timing sotto":"");
    if(calls&&getenv("TIN_CUDA_PROFILE")) fprintf(stderr,
...
static int parse_cuda_devices(const char *list, int *out){
    ...
    while(*p){
        ...
        if(end==p||v<0||v>INT_MAX||n>=TIN_CUDA_MAX_DEVICES) return 0;
 ...
 * this keeps tinllm dependency-free while removing one team launch/expert. */
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    for(int i=0;i<c->n_layers;i++){
        ...
        if(getenv("TIN_CUDA_ATTN_SHARD")&&atoi(getenv("TIN_CUDA_ATTN_SHARD")))
...
#ifdef TIN_CUDA
static int g_cuda_pipe=0;   /* TIN_CUDA_PIPE=1: prefill attention chain resident on the layer home device */
...
static int attn_pipe_prefill(Model *m, Layer *l, int layer, const float *x, int x_is_dev,
    ...
    if(out_dev && l->n_kv_b_shard>1 &&
       getenv("TIN_CUDA_PIPE_SHARD") && atoi(getenv("TIN_CUDA_PIPE_SHARD"))){
...
static void attention_rows(Model *m, Layer *l, int layer, float *x, int S, int pos_base,
    ...
    int cuda_absorb=0;
#ifdef TIN_CUDA
    cuda_absorb=layer<c->n_layers&&!kvs&&g_cuda_enabled&&getenv("TIN_CUDA_ATTN")&&
                atoi(getenv("TIN_CUDA_ATTN"))&&c->kv_lora<=512;
#endif
    ...
    if(absorb && c->kv_lora<=512){
        ...
        } else if(S<=4&&g_cuda_enabled&&getenv("TIN_CUDA_ATTN")&&atoi(getenv("TIN_CUDA_ATTN"))&&
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out, int with_shared){
...
#ifdef TIN_CUDA
        ColiCudaTensor *dev_g[TIN_CUDA_MAX_DEVICES][64],*dev_u[TIN_CUDA_MAX_DEVICES][64];
        ColiCudaTensor *dev_d[TIN_CUDA_MAX_DEVICES][64];
        int dev_rows[TIN_CUDA_MAX_DEVICES][64],dev_which[TIN_CUDA_MAX_DEVICES][64];
        int dev_nc[TIN_CUDA_MAX_DEVICES]={0},dev_total[TIN_CUDA_MAX_DEVICES]={0};
        int dev_off[TIN_CUDA_MAX_DEVICES]={0},dev_ok[TIN_CUDA_MAX_DEVICES]={0};
...
#endif
#ifdef TIN_CUDA
    int shared_min=getenv("TIN_CUDA_SHARED_W4A16_MIN_ROWS")?
        atoi(getenv("TIN_CUDA_SHARED_W4A16_MIN_ROWS")):32;
    ...
    if(shared_cuda==0&&S>=shared_min&&!l->shared_w4a16_failed&&!omp_in_parallel()&&g_cuda_enabled&&
       ...
       getenv("TIN_CUDA_SHARED_W4A16")&&atoi(getenv("TIN_CUDA_SHARED_W4A16"))&&
...
static void layers_forward_rows(Model *m, float *x, int S, int pos_base,
     ...
     * TIN_CUDA_PIPE_S_MIN overrides for anyone who wants to measure. */
    float *x_dev=NULL; int x_dev_on=-1;
    size_t xb=(size_t)S*(size_t)D*4;
    int pipe_s_min = getenv("TIN_CUDA_PIPE_S_MIN") ? atoi(getenv("TIN_CUDA_PIPE_S_MIN"))
                                                     : (g_cuda_ndev<=1 ? 1 : 8);
 ...
 * piu' usi tinllm', meglio l'auto-pin conosce i TUOI expert caldi. */
...
/* HOT-STORE ("il redis del tinllm'"): carica in RAM, UNA VOLTA e per sempre, i top expert
...
static void pin_load(Model *m, const char *statspath, double gb){
    ...
    double t0=now_s();
#ifdef TIN_CUDA
    double remaining[TIN_CUDA_MAX_DEVICES]={0}, placed_b[TIN_CUDA_MAX_DEVICES]={0};
    int placed_n[TIN_CUDA_MAX_DEVICES]={0}, gpu_prefix=0;
    ...
    if(g_cuda_enabled && budget>0){
        ...
        for(int a=0;a<gpu_limit && m->gpu_expert_bytes<budget;a++){
            ...
            { ESlot *s=&m->pin[li][slot_of[a]];
                ...
                int tried[TIN_CUDA_MAX_DEVICES]={0}, placed=0;
...
int main(int argc, char **argv){
    ...
    if(!getenv("COLI_OMP_TUNED") && !getenv("COLI_NO_OMP_TUNE") &&
       !getenv("TIN_CUDA") && !getenv("TIN_METAL")){
...
#ifdef TIN_CUDA
    if(getenv("TIN_CUDA") && atoi(getenv("TIN_CUDA"))){
        const char *one=getenv("TIN_GPU"), *many=getenv("TIN_GPUS");
        if(one&&many){ fprintf(stderr,"use TIN_GPU or TIN_GPUS, not both\n"); return 2; }
        ...
        if(g_cuda_ndev<1){ fprintf(stderr,"invalid TIN_GPUS: use a list such as 0,1,2\n"); return 2; }
    ...
    g_cuda_pipe=getenv("TIN_CUDA_PIPE")?atoi(getenv("TIN_CUDA_PIPE")):0;
    ...
    if((getenv("TIN_GPU")||getenv("TIN_GPUS"))&&!g_cuda_enabled){ fprintf(stderr,"TIN_GPU(S) requires TIN_CUDA=1\n"); return 2; }
    if(g_cuda_dense&&!g_cuda_enabled){ fprintf(stderr,"CUDA_DENSE requires TIN_CUDA=1\n"); return 2; }
    if((g_cuda_expert_gb>0||g_cuda_expert_auto) && !g_cuda_enabled){ fprintf(stderr,"CUDA_EXPERT_GB requires TIN_CUDA=1\n"); return 2; }
...
#else
    if((getenv("TIN_CUDA") && atoi(getenv("TIN_CUDA"))) ||
       getenv("TIN_GPU") || getenv("TIN_GPUS") ||
...
#endif
#ifdef TIN_METAL
    if(getenv("TIN_METAL") && atoi(getenv("TIN_METAL"))){
        ...
        if(getenv("TIN_METAL_SPIN") && atoi(getenv("TIN_METAL_SPIN"))){ coli_metal_spin_start(); fprintf(stderr,"[METAL] keep-alive spinner ON\n"); }
        if(getenv("TIN_METAL_GEMM_MIN")) g_metal_gemm_min=atoi(getenv("TIN_METAL_GEMM_MIN"));
    }
#else
    if(getenv("TIN_METAL") && atoi(getenv("TIN_METAL"))){
    ...
    if(g_draft<0){
         ...
         * always exists on a single 16 GB card. TIN_CUDA_MTP=1 opts in for
        ...
        int cuda_mtp = getenv("TIN_CUDA_MTP") ? atoi(getenv("TIN_CUDA_MTP")) : 0;
    ...
    { int maxid=0; for(int i=0;i<nfull;i++) if(full[i]>maxid) maxid=full[i];
      if(m.c.vocab>1000 && maxid<1000 && !getenv("REF_FORCE")){
        fprintf(stderr,
          ...
          "         or:                python tin chat --model <model>\n"
