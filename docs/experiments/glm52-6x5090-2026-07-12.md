...
This note records the July 12, 2026 experiments that compared tinllm with
...
The day had two halves. The morning runs (below) measured tinllm at 0.12
tok/s and vLLM-Moet at 2.3-2.7 tok/s. The afternoon full-resident work on
tinllm (see "tinllm full-resident experiments") removed disk from the decode
...
- tinllm INT4 model: `/data/models/GLM-5.2-tinllm-int4` (144 shards)
...
| --- | --- | ---: | ---: | --- |
| tinllm INT4 (morning, partial residency) | 6 GPUs | about 42 s | 0.12 tok/s | GPU hot tier was active, but dense compute remained on CPU |
| tinllm INT4 (afternoon, full residency) | 6 GPUs + 24 cores | — | 6.28-6.84 tok/s | all 19,456 experts resident in VRAM+RAM, zero disk waits |
...
The morning tinllm run generated 16 tokens in 132.46 seconds. Its measured
...
## tinllm full-resident experiments (afternoon)

Test directory: `/data/test/tinllm-full-resident`. Related PR:
[#80](https://github.com/JustVugg/tinllm/pull/80) (on top of PR #68). All A/B
...
(`mateogrgic/GLM-5.2-tinllm-int4-with-int8-mtp`, ~10 GB; originals kept as
...
Logs: `/data/test/tinllm-six-gpu-ab-mtp-*-20260712.log` (phase 1),
`/data/test/tinllm-six-gpu-ab-mtp8-*-20260713.log` (phase 2).
...
`/data/test/tinllm-six-gpu-ab-avx512q-*-20260713.log`,
...
model fits in VRAM+RAM, and once disk left the decode path tinllm at
6.28-6.84 tok/s beat every vLLM-Moet layout measured here (best 2.6 tok/s),
because tinllm never replays a whole step on an expert miss.
...
The fastest configuration overall is tinllm full-resident in
`/data/test/tinllm-full-resident` (6.28-6.84 tok/s; stop the
...
reproduced. Raw A/B logs live under `/data/test/tinllm-six-gpu-ab-*.log`.
